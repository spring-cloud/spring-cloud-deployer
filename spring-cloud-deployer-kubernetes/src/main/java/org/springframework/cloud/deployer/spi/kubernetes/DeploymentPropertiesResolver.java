...
class DeploymentPropertiesResolver {
    ...
    private @Nullable Container initContainerFromProperties(Map<String, String> kubeProps, String propertyKey) {
        ...
        if (!StringUtils.hasText(name)) {
            name = PropertyParserUtils.getDeploymentPropertyValue(kubeProps, propertyKey + ".containerName");
        }

        String image = PropertyParserUtils.getDeploymentPropertyValue(kubeProps, propertyKey + ".image");
        if (!StringUtils.hasText(image)) {
            image = PropertyParserUtils.getDeploymentPropertyValue(kubeProps, propertyKey + ".imageName");
        }
        if (StringUtils.hasText(name) && StringUtils.hasText(image)) {
            ...
            if (!StringUtils.hasText(commandStr)) {
                commandStr = PropertyParserUtils.getDeploymentPropertyValue(kubeProps, propertyKey + ".commands");
            }
            ...
            return new ContainerBuilder()
                    ...
                    .withArgs(args)
                    .withImagePullPolicy(PropertyParserUtils.getDeploymentPropertyValue(kubeProps, propertyKey + "imagePullPolicy"))
                    .withWorkingDir(PropertyParserUtils.getDeploymentPropertyValue(kubeProps, propertyKey + "workingDir"))
    ...
    private Container containerFromProps(InitContainer initContainerProps) {
        return new ContainerBuilder()
                ...
                .withImagePullPolicy(initContainerProps.getImagePullPolicy())
                .withWorkingDir(initContainerProps.getWorkingDir())
